...
template <class T>
void QuikSort(vector<T>& arr, int start, int finish) {
	if (finish - start <= 1) return;

	static mt19937 generator(42); // генератор ћерсенна “вистера с фиксированным сидом
	uniform_int_distribution<int> dist(start, finish - 1);
	T x = arr[dist(generator)];

	vector<int> lmn = Partition<T>(arr, start, finish, x);
	QuikSort(arr, start, start + lmn[0]);
	QuikSort(arr, start + lmn[0] + lmn[1], finish);
}


int main() {
	vector<int> vec = { 3, 8, 4, -4, 8, 7, 6, 5 };
	QuikSort(vec, 0, vec.size());
